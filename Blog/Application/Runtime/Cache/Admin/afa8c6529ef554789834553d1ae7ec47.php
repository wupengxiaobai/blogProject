...
<body>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                <?php if(is_array($data)): $key = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr>
                        <td><?php echo ($key); ?></td>
                        <td><?php echo (msubstr($vo["content"],0,12)); ?></td>
                        <td><?php echo ($vo["mobilephone"]); ?></td>
                        <td><?php echo (msubstr($vo["email"],0,11)); ?></td>
                        <td><?php echo (date('Y-m-d H:i:s',$vo["addtime"])); ?></td>
                        <td>
                            <?php if($vo["islook"] == 0): ?><span style="color:red;">未查看</span>
                                <?php else: ?><span style="color:forestgreen">已查看</span><?php endif; ?>
                        </td>
                        <td>
                            <button class="del btn btn-danger btn-xs" onClick="delConfirm(<?php echo ($vo['id']); ?>)">删除</button>
                            <a href="<?php echo U('see',array('id'=>$vo['id']));?>" class="btn btn-info btn-xs">查看</a>
                        </td>
    ...
    <script>
        function delConfirm($id) {
            ...
            if (delOk) {
                window.location.href = '/index.php/Admin/Book/del/id/' + $id;
            }
        }
    </script>
</body>
